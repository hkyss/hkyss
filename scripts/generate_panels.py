#!/usr/bin/env python3
"""Render a GitHub profile as a set of matching Vault-Tec terminal panels."""
...
def panel(height: int, title_left: str, title_right: str, body: str, *,
          css: str = "", defs: str = "", footer: tuple[str, str] | None = None,
          aria: str = "") -> str:
    foot = ""
    if footer:
        foot_left, foot_right = footer
        rule_y = height - 34
        text_y = height - 16
        foot = (
            f'<rect x="{PAD}" y="{rule_y}" width="{WIDTH - PAD * 2}" height="1"'
            f' fill="{FRAME}" opacity=".35"/>'
            + label(foot_left + "_", PAD, text_y, FRAME, glow=True,
                    inner=f'<tspan>{html.escape(foot_left)}</tspan>'
                          f'<tspan class="caret">_</tspan>')
            + right(foot_right, WIDTH - PAD, text_y, DIM)
        )
...
{label(title_left, PAD, TITLE_Y, FRAME, glow=True)}
{right(title_right, WIDTH - PAD, TITLE_Y, DIM)}
<rect x="{PAD}" y="{RULE_Y}" width="{WIDTH - PAD * 2}" height="1" fill="{FRAME}" opacity=".35"/>
{body}
{foot}
...
def scrape_calendar(user: str) -> dict:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "vault-tec-panels"})
    with urllib.request.urlopen(req, timeout=30) as resp:
    ...
    days = [{"date": d, "row": int(r), "col": int(c), "level": int(l), "count": int(l)}
...
def render_header(user: str, profile: dict | None, repos: str) -> str:
    height = 178
    name = user.upper()
    cx, cy, r = WIDTH - PAD - 42, 96, 40
    ...
    emblem = (
        ...
        f'{spokes}</g>'
        f'<circle cx="{cx}" cy="{cy}" r="3.2" fill="{FRAME}"/>'
    )

    since = profile["createdAt"][:10] if profile else "----------"
    boot = f"> SYSTEM BOOT COMPLETE :: {repos} REPOSITORIES ONLINE :: REG {since} "

    body = (
        label(name + "_", PAD, 104, FRAME, size=44, glow=True, weight="700",
              inner=f'<tspan>{html.escape(name)}</tspan><tspan class="caret">_</tspan>')
        + label("> ROLE: DEVELOPER", PAD, 132, DIM, size=12)
        + emblem
    )
    return panel(
        height, "ROBCO INDUSTRIES (TM) TERMLINK PROTOCOL", "PIPBOY 3000 MK IV", body,
        css=f"#cog{{animation:spin 24s linear infinite;transform-origin:{cx}px {cy}px}}"
            "@keyframes spin{from{transform:rotate(0)}to{transform:rotate(360deg)}}",
        footer=(boot, "LINK ESTABLISHED"),
        aria=f"{user} terminal header",
    )


def render_scan(user: str, cal: dict) -> str:
    days = cal["days"]
    ...
    for col in range(cols):
        ...
        if month != last_month and col - last_col >= 3 and x + run_width(text, 9) <= WIDTH - PAD:
            months.append(label(text, x, 66, DIM, size=9))
    ...
    return panel(
        height, "ARCHIVE :: CONTRIBUTION SCAN", f"{dates[0]} / {dates[-1]}", body,
        css="".join(css),
        defs=f'<radialGradient id="lamp"><stop offset="0" stop-color="{FRAME}" stop-opacity=".38"/>'
        ...
        footer=(f"> {total} ENTRIES / {logged} ACTIVE DAYS ",
                f"STREAK {current} :: MAX {longest}"),
        aria=f"{user} contribution archive",
    )


def render_status(profile: dict | None, cal: dict) -> str:
    height = 252
    mid = WIDTH / 2
    left_edge, right_start = WIDTH - PAD, mid + 30
    ...
    rows = [label("> SYSTEM STATS", PAD, 70, FRAME, size=11, glow=True),
            label("> LANGUAGE ANALYSIS", right_start, 70, FRAME, size=11, glow=True),
            f'<rect x="{mid}" y="58" width="1" height="152" fill="{FRAME}" opacity=".25"/>']
    ...
    css = ["@keyframes fill{0%{transform:scaleX(0)}16%,94%{transform:scaleX(1)}"
           "100%{transform:scaleX(0)}}"]
    for i, (name, share) in enumerate(langs):
        y = 98 + i * 21
        clipped = name[:12]
        rows.append(label(clipped, right_start, y, DIM))
        rows.append(right(f"{share:.1f}%", left_edge, y, FRAME))
        ...
        rows.append(f'<rect x="{bar_x}" y="{y - 8}" width="{bar_w}" height="8" rx="2"'
                    f' fill="url(#seg)"/>')
    css.append(".bar{animation:fill 9s ease-out infinite}")

    if not langs:
        rows.append(label("NO DATA", right_start, 98, DIM))

    return panel(
        height, "SYSTEM DIAGNOSTIC", "V.A.T.S. READOUT", "".join(rows),
        css="".join(css),
        footer=("> DIAGNOSTIC COMPLETE :: ALL SYSTEMS OPERATIONAL ",
                "NO ANOMALIES DETECTED"),
        defs=f'<pattern id="seg" width="8" height="8" patternUnits="userSpaceOnUse">'
             f'<rect x="6" width="2" height="8" fill="{BG}"/></pattern>',
        aria="profile statistics and language breakdown",
    )
...
def main() -> None:
    ...
    for name, svg in (("header.svg", render_header(args.user, profile, repos)),
                      ("scan.svg", render_scan(args.user, cal)),
                      ("status.svg", render_status(profile, cal))):
        (out / name).write_text(svg, encoding="utf-8")
        print(f"wrote {out / name} ({len(svg) / 1024:.0f} KB)")
